...
See syntax at [Mongodb]({{< relref "03. Enumeration/27017 TCP - Mongodb#syntax" >}})
...
> Js common sense: if there are multiple `||` and `&&`, js evaluates `&&` first. If there is `()`, js evaluates inside `()` first.
> eg. `false || true || true && false` = `false || true || false` = `true`
