...
> NOTE: Sometimes, we supply a name, and it's reflected in the `Subject` header like this: `You received a message from <name>!`.
> In this case, `!` is appended to our input. If we try to inject a `Cc` header containing our email address, the web application will append the exclamation mark to our email address and thus invalidate it. It is therefore **recommended always to inject an additional dummy header after our actual payload** to prevent such issues
