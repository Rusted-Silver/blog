...
`Serialization` is the process of taking an object from memory and converting it into a series of bytes so that it can be stored or transmitted over a network and then reconstructed later on.
`Deserialization` is the reverse action: taking serialized data and reconstructing the original object
