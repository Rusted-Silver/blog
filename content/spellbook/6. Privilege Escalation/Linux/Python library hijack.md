...
> Note: if the capability has value `=eip`, which means the child process that this executable spawn also `inherit` the capbability. Then we can also do path injection. See [Path Injection]({{< relref "6. Privilege Escalation/Linux/Path Injection#executable-python-script" >}})
